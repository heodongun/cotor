...
- Company defaults should bias toward cost control: new company-seeded execution agents should prefer OpenCode with the default model `opencode/minimax-m2.5-free` unless the repo intentionally changes that product policy.
...
- 회사 기본 에이전트 정책은 비용 통제를 우선합니다. 저장소가 의도적으로 바꾸지 않는 한 새 회사에 시드되는 실행 에이전트는 기본 모델 `opencode/minimax-m2.5-free`를 사용하는 OpenCode를 우선합니다.

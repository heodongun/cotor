...
Paperclip 같은 캔버스 도구가 빠른 실험에 적합하다면, Cotor 웹 에디터는 그 결과를 버전 관리 가능한 실행 자산으로 고정하는 데 초점을 둡니다. 에디터에서 만든 파이프라인은 `.cotor/web` 아래 YAML로 저장되며, CLI와 동일한 검증/실행 흐름으로 이어집니다.
